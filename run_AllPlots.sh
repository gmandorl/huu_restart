...
# g++ ./plotter_vbfzll.C -g -o plot `root-config --cflags --glibs`   -lMLP -lXMLIO -lTMVA  
# g++ ./plotter_vbfzll_GIULIO.C -g -o plot `root-config --cflags --glibs`   -lMLP -lXMLIO -lTMVA  
...
echo run_plotter_parallel.sh $QCDcorrection $JEScorrection $JERcorrection $PUcorrection $executable $v $lab;
